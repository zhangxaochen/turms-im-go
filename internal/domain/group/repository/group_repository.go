...
// QueryGroupsWithFullFilter retrieves groups based on all filter parameters matching Java's queryGroups.
func (r *GroupRepository) QueryGroupsWithFullFilter(
	ctx context.Context,
	ids, typeIds, creatorIds, ownerIds []int64,
	isActive *bool,
	creationDateStart, creationDateEnd, deletionDateStart, deletionDateEnd, muteEndDateStart, muteEndDateEnd *time.Time,
	skip *int32, limit *int32,
) ([]*po.Group, error) {
	filter := bson.M{}
	if len(ids) > 0 {
		filter["_id"] = bson.M{"$in": ids}
