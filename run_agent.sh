...
while [ $count -lt $MAX_RETRIES ]; do
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        ...
        git add .
        git commit -m "fix(automation): resolve parity bugs for batch 12" || true
        ...
        while true; do
            ...
            (
                flock -x 200
                echo "[$(date)] Attempting merge for feature/fix-batch-12 into main..."
                if git merge "feature/fix-batch-12" --no-edit -m "Merge auto-fix batch 12 into main"; then
                    echo "SUCCESS" > .git/merge_result_batch_12
                else
                    echo "[!] Conflict detected. Aborting merge."
                    git merge --abort
                    echo "CONFLICT" > .git/merge_result_batch_12
                fi
            ) 200>.git/merge_lock.lock

            MERGE_RESULT=$(cat .git/merge_result_batch_12)
            rm -f .git/merge_result_batch_12

            if [ "$MERGE_RESULT" = "SUCCESS" ]; then
                echo "[$(date)] Successfully merged batch 12 into main."
                break
            fi

            echo "[$(date)] Merge conflict! Initiating Sub-Agent Self-Heal Rebase..."
            cd "/Users/11176728/gemini-cli/dev-turms-im-refactor/turms-worker-batch-12"

            # 开始 Rebase main
            git rebase main || {
                # 触发大模型自动修复冲突
                echo "[$(date)] Handing over to Claude for conflict resolution..."
                claude -p --dangerously-skip-permissions "A git rebase conflict was detected in feature/fix-batch-12.
...
Do NOT attempt to run standard git merge. Finish the rebase process. Keep your thoughts and logs concise."
                ...
                # 检查大模型是否成功继续了 rebase
                if git rebase --show-current-patch >/dev/null 2>&1 || [ -d "/Users/11176728/gemini-cli/dev-turms-im-refactor/turms-worker-batch-12/.git/rebase-merge" ]; then
        ...
        echo "[$(date)] Pipeline for batch 12 complete."
        exit 0
    fi
